...
class JournalService:
    ...
    def list_records(self, session: Session) -> list[IncidentRecordResponse]:
        ...
        return [
            ...
            for item in self.repository.list_records(session, user.id)
        ]

    def create_record(
        self, session: Session, payload: IncidentRecordCreate
    ) -> IncidentRecordResponse:
        user = self._current_user(session)
    ...
    def generate_summary(
        ...
        if record.perceived_impacts:
            details.append(
                f"Impactos percebidos: {', '.join(record.perceived_impacts)}."
            )
